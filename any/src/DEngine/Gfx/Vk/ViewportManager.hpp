...
namespace DEngine::Gfx::Vk
{
	struct ViewportMgr_GfxRenderTarget {
		vk::Extent2D extent = {};
		BoxVmaImg colorImg;
		BoxVkImageView colorImgView;
		BoxVmaImg stencilImg;
		BoxVkImageView stencilImgView;
		BoxVkFramebuffer framebuffer;

		[[nodiscard]] bool IsInitialized() const { return !colorImg.IsNull(); }
	};
	...
	struct ViewportManager {
		static constexpr vk::DescriptorType cameraDataUniformDescrType = vk::DescriptorType::eUniformBuffer;
		// TODO: We should likely make a struct that matches the Slang equivalent.
		static constexpr uSize cameraDataUniformStructSize = sizeof(Math::Mat4);
