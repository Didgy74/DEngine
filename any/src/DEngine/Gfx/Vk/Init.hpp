...
namespace DEngine::Gfx::Vk::Init
{
	...
	[[nodiscard]] Std::StackVec<BoxVkFence, Constants::maxInFlightCount> CreateMainFences(
		DevDispatch const& device,
		u8 resourceSetCount,
		DebugUtilsDispatch const* debugUtils);

	struct CreateMainCmdBuffers_ReturnT {
		Std::StackVec<BoxVkCommandPool, Const::maxInFlightCount> cmdPools;
	...
	// We need to generate the Gui RenderPass before GuiResourceManager because
	// the relevant VkFramebuffers are managed by the NativeWinMgr
