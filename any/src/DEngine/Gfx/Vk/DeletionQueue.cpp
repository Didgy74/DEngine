...
void Vk::DeletionQueue::Destroy(BoxVmaImg&& in)
{
	auto temp = in.Release();
	Destroy(temp.alloc, temp.handle);
}

void Vk::DeletionQueue::Destroy(VmaAllocation vmaAlloc, vk::Buffer buffer)
{
	DENGINE_IMPL_GFX_ASSERT(vmaAlloc != nullptr);
...
void Vk::DeletionQueue::Destroy(BoxVmaBuffer&& in)
{
...
void DeletionQueue::Destroy(BoxVkImageView&& in) {
	impl::DeletionQueueImpl::DestroyDeviceLevelHandle(*this, in.Release());
}
...
void DeletionQueue::Destroy(vk::Semaphore in) {
	impl::DeletionQueueImpl::DestroyDeviceLevelHandle(*this, in);
}

void DeletionQueue::Destroy(BoxVkSemaphore&& in) {
	impl::DeletionQueueImpl::DestroyDeviceLevelHandle(*this, in.Release());
}
