...
PFN_vkGetInstanceProcAddr Vk::loadInstanceProcAddressPFN()
{
	...
	// TODO: Need to implement closing of this dylib.
	...
	void* lib = dlopen("./lib/libvulkan.dylib", RTLD_NOW | RTLD_LOCAL);
	if (lib == nullptr)
		throw std::runtime_error("Vulkan: Unable to load libvulkan.dylib for dynamic dispatching");
	procAddr = (PFN_vkGetInstanceProcAddr)dlsym(lib, "vkGetInstanceProcAddr");
	//dlclose(lib);
#endif
...
[[nodiscard]] static auto PfnLoaderBaseHelper(
	PFN_vkGetInstanceProcAddr getInstanceProcAddr,
	char const* name)
{
	DENGINE_IMPL_GFX_ASSERT(getInstanceProcAddr != nullptr);
	DENGINE_IMPL_GFX_ASSERT(name != nullptr);
	auto temp = getInstanceProcAddr(nullptr, name);
	DENGINE_IMPL_GFX_ASSERT(temp != nullptr);
	return temp;
}

[[nodiscard]] static auto PfnLoaderInstanceHelper(
	PFN_vkGetInstanceProcAddr getInstanceProcAddr,
	vk::Instance instance,
	char const* name)
{
	DENGINE_IMPL_GFX_ASSERT(getInstanceProcAddr != nullptr);
	DENGINE_IMPL_GFX_ASSERT(instance != vk::Instance{});
	DENGINE_IMPL_GFX_ASSERT(name != nullptr);
	auto temp = getInstanceProcAddr(instance, name);
	DENGINE_IMPL_GFX_ASSERT(temp != nullptr);
	return temp;
}

[[nodiscard]] static auto PfnLoaderDeviceHelper(
	PFN_vkGetDeviceProcAddr getDeviceProcAddr,
	vk::Device device,
	char const* name)
{
	DENGINE_IMPL_GFX_ASSERT(getDeviceProcAddr != nullptr);
	DENGINE_IMPL_GFX_ASSERT(device != vk::Device{});
	DENGINE_IMPL_GFX_ASSERT(name != nullptr);
	auto temp = getDeviceProcAddr(device, name);
	DENGINE_IMPL_GFX_ASSERT(temp != nullptr);
	return temp;
}
...
BaseDispatchRaw BaseDispatchRaw::Build(PFN_vkGetInstanceProcAddr getInstanceProcAddr)
{
	...
	returnVal.vkGetInstanceProcAddr = getInstanceProcAddr;
	returnVal.vkCreateInstance = (PFN_vkCreateInstance)PfnLoaderBaseHelper(getInstanceProcAddr, "vkCreateInstance");
	returnVal.vkEnumerateInstanceExtensionProperties = (PFN_vkEnumerateInstanceExtensionProperties)PfnLoaderBaseHelper(getInstanceProcAddr, "vkEnumerateInstanceExtensionProperties");
	returnVal.vkEnumerateInstanceLayerProperties = (PFN_vkEnumerateInstanceLayerProperties)PfnLoaderBaseHelper(getInstanceProcAddr, "vkEnumerateInstanceLayerProperties");
	returnVal.vkEnumerateInstanceVersion = (PFN_vkEnumerateInstanceVersion)PfnLoaderBaseHelper(getInstanceProcAddr, "vkEnumerateInstanceVersion");
	return returnVal;
}
...
InstanceDispatchRaw InstanceDispatchRaw::Build(
	...
	PFN_vkGetInstanceProcAddr getInstanceProcAddr)
{
	auto fn = [&](char const* in) {
		return PfnLoaderInstanceHelper(getInstanceProcAddr, instance, in);
	};
	InstanceDispatchRaw returnVal = {};
	returnVal.vkCreateDevice = (PFN_vkCreateDevice)fn("vkCreateDevice");
	returnVal.vkDestroyInstance = (PFN_vkDestroyInstance)fn("vkDestroyInstance");
	returnVal.vkEnumeratePhysicalDevices = (PFN_vkEnumeratePhysicalDevices)fn( "vkEnumeratePhysicalDevices");
	returnVal.vkEnumerateDeviceExtensionProperties = (PFN_vkEnumerateDeviceExtensionProperties)fn("vkEnumerateDeviceExtensionProperties");
	returnVal.vkGetPhysicalDeviceFeatures = (PFN_vkGetPhysicalDeviceFeatures)fn("vkGetPhysicalDeviceFeatures");
	returnVal.vkGetPhysicalDeviceMemoryProperties = (PFN_vkGetPhysicalDeviceMemoryProperties)fn("vkGetPhysicalDeviceMemoryProperties");
	returnVal.vkGetPhysicalDeviceProperties = (PFN_vkGetPhysicalDeviceProperties)fn("vkGetPhysicalDeviceProperties");
	returnVal.vkGetPhysicalDeviceQueueFamilyProperties = (PFN_vkGetPhysicalDeviceQueueFamilyProperties)fn("vkGetPhysicalDeviceQueueFamilyProperties");
	return returnVal;
}
	...
	PFN_vkGetDeviceProcAddr getDeviceProcAddr)
{
	...
	returnVal.vkAllocateCommandBuffers = (PFN_vkAllocateCommandBuffers)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkAllocateCommandBuffers");
	returnVal.vkAllocateDescriptorSets = (PFN_vkAllocateDescriptorSets)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkAllocateDescriptorSets");
	returnVal.vkAllocateMemory = (PFN_vkAllocateMemory)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkAllocateMemory");
	returnVal.vkBeginCommandBuffer = (PFN_vkBeginCommandBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkBeginCommandBuffer");
	returnVal.vkBindBufferMemory = (PFN_vkBindBufferMemory)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkBindBufferMemory");
	returnVal.vkBindImageMemory = (PFN_vkBindImageMemory)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkBindImageMemory");
	returnVal.vkCmdBeginRenderPass = (PFN_vkCmdBeginRenderPass)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdBeginRenderPass");
	returnVal.vkCmdBindDescriptorSets = (PFN_vkCmdBindDescriptorSets)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdBindDescriptorSets");
	returnVal.vkCmdBindIndexBuffer = (PFN_vkCmdBindIndexBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdBindIndexBuffer");
	returnVal.vkCmdBindPipeline = (PFN_vkCmdBindPipeline)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdBindPipeline");
	returnVal.vkCmdBindVertexBuffers = (PFN_vkCmdBindVertexBuffers)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdBindVertexBuffers");
	returnVal.vkCmdCopyBuffer = (PFN_vkCmdCopyBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdCopyBuffer");
	returnVal.vkCmdCopyBufferToImage = (PFN_vkCmdCopyBufferToImage)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdCopyBufferToImage");
	returnVal.vkCmdCopyImage = (PFN_vkCmdCopyImage)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdCopyImage");
	returnVal.vkCmdCopyImageToBuffer = (PFN_vkCmdCopyImageToBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdCopyImageToBuffer");
	returnVal.vkCmdDraw = (PFN_vkCmdDraw)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdDraw");
	returnVal.vkCmdDrawIndexed = (PFN_vkCmdDrawIndexed)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdDrawIndexed");
	returnVal.vkCmdEndRenderPass = (PFN_vkCmdEndRenderPass)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdEndRenderPass");
	returnVal.vkCmdNextSubpass = (PFN_vkCmdNextSubpass)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdNextSubpass");
	returnVal.vkCmdPipelineBarrier = (PFN_vkCmdPipelineBarrier)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdPipelineBarrier");
	returnVal.vkCmdPushConstants = (PFN_vkCmdPushConstants)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdPushConstants");
	returnVal.vkCmdSetScissor = (PFN_vkCmdSetScissor)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdSetScissor");
	returnVal.vkCmdSetStencilReference = (PFN_vkCmdSetStencilReference)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdSetStencilReference");
	returnVal.vkCmdSetViewport = (PFN_vkCmdSetViewport)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCmdSetViewport");
	returnVal.vkCreateBuffer = (PFN_vkCreateBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateBuffer");
	returnVal.vkCreateBufferView = (PFN_vkCreateBufferView)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateBufferView");
	returnVal.vkCreateCommandPool = (PFN_vkCreateCommandPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateCommandPool");
	returnVal.vkCreateDescriptorPool = (PFN_vkCreateDescriptorPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateDescriptorPool");
	returnVal.vkCreateDescriptorSetLayout = (PFN_vkCreateDescriptorSetLayout)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateDescriptorSetLayout");
	returnVal.vkCreateFence = (PFN_vkCreateFence)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateFence");
	returnVal.vkCreateFramebuffer = (PFN_vkCreateFramebuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateFramebuffer");
	returnVal.vkCreateGraphicsPipelines = (PFN_vkCreateGraphicsPipelines)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateGraphicsPipelines");
	returnVal.vkCreateImage = (PFN_vkCreateImage)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateImage");
	returnVal.vkCreateImageView = (PFN_vkCreateImageView)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateImageView");
	returnVal.vkCreatePipelineCache = (PFN_vkCreatePipelineCache)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreatePipelineCache");
	returnVal.vkCreatePipelineLayout = (PFN_vkCreatePipelineLayout)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreatePipelineLayout");
	returnVal.vkCreateRenderPass = (PFN_vkCreateRenderPass)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateRenderPass");
	returnVal.vkCreateSampler = (PFN_vkCreateSampler)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateSampler");
	returnVal.vkCreateSemaphore = (PFN_vkCreateSemaphore)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateSemaphore");
	returnVal.vkCreateShaderModule = (PFN_vkCreateShaderModule)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkCreateShaderModule");
	returnVal.vkDestroyBuffer = (PFN_vkDestroyBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyBuffer");
	returnVal.vkDestroyBufferView = (PFN_vkDestroyBufferView)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyBufferView");
	returnVal.vkDestroyCommandPool = (PFN_vkDestroyCommandPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyCommandPool");
	returnVal.vkDestroyDescriptorPool = (PFN_vkDestroyDescriptorPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyDescriptorPool");
	returnVal.vkDestroyDescriptorSetLayout = (PFN_vkDestroyDescriptorSetLayout)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyDescriptorSetLayout");
	returnVal.vkDestroyDevice = (PFN_vkDestroyDevice)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyDevice");
	returnVal.vkDestroyFence = (PFN_vkDestroyFence)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyFence");
	returnVal.vkDestroyFramebuffer = (PFN_vkDestroyFramebuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyFramebuffer");
	returnVal.vkDestroyImage = (PFN_vkDestroyImage)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyImage");
	returnVal.vkDestroyImageView = (PFN_vkDestroyImageView)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyImageView");
	returnVal.vkDestroyPipeline = (PFN_vkDestroyPipeline)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyPipeline");
	returnVal.vkDestroyPipelineCache = (PFN_vkDestroyPipelineCache)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyPipelineCache");
	returnVal.vkDestroyPipelineLayout = (PFN_vkDestroyPipelineLayout)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyPipelineLayout");
	returnVal.vkDestroyRenderPass = (PFN_vkDestroyRenderPass)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyRenderPass");
	returnVal.vkDestroySampler = (PFN_vkDestroySampler)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroySampler");
	returnVal.vkDestroySemaphore = (PFN_vkDestroySemaphore)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroySemaphore");
	returnVal.vkDestroyShaderModule = (PFN_vkDestroyShaderModule)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDestroyShaderModule");
	returnVal.vkDeviceWaitIdle = (PFN_vkDeviceWaitIdle)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkDeviceWaitIdle");
	returnVal.vkEndCommandBuffer = (PFN_vkEndCommandBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkEndCommandBuffer");
	returnVal.vkFlushMappedMemoryRanges = (PFN_vkFlushMappedMemoryRanges)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkFlushMappedMemoryRanges");
	returnVal.vkFreeCommandBuffers = (PFN_vkFreeCommandBuffers)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkFreeCommandBuffers");
	returnVal.vkFreeDescriptorSets = (PFN_vkFreeDescriptorSets)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkFreeDescriptorSets");
	returnVal.vkFreeMemory = (PFN_vkFreeMemory)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkFreeMemory");
	returnVal.vkGetBufferMemoryRequirements = (PFN_vkGetBufferMemoryRequirements)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkGetBufferMemoryRequirements");
	returnVal.vkGetDeviceProcAddr = (PFN_vkGetDeviceProcAddr)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkGetDeviceProcAddr");
	returnVal.vkGetDeviceQueue = (PFN_vkGetDeviceQueue)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkGetDeviceQueue");
	returnVal.vkGetFenceStatus = (PFN_vkGetFenceStatus)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkGetFenceStatus");
	returnVal.vkGetImageMemoryRequirements = (PFN_vkGetImageMemoryRequirements)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkGetImageMemoryRequirements");
	returnVal.vkInvalidateMappedMemoryRanges = (PFN_vkInvalidateMappedMemoryRanges)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkInvalidateMappedMemoryRanges");
	returnVal.vkMapMemory = (PFN_vkMapMemory)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkMapMemory");
	returnVal.vkResetCommandBuffer = (PFN_vkResetCommandBuffer)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkResetCommandBuffer");
	returnVal.vkResetCommandPool = (PFN_vkResetCommandPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkResetCommandPool");
	returnVal.vkResetDescriptorPool = (PFN_vkResetDescriptorPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkResetDescriptorPool");
	returnVal.vkResetFences = (PFN_vkResetFences)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkResetFences");
	returnVal.vkTrimCommandPool = (PFN_vkTrimCommandPool)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkTrimCommandPool");
	returnVal.vkUnmapMemory = (PFN_vkUnmapMemory)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkUnmapMemory");
	returnVal.vkUpdateDescriptorSets = (PFN_vkUpdateDescriptorSets)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkUpdateDescriptorSets");
	returnVal.vkWaitForFences = (PFN_vkWaitForFences)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkWaitForFences");
	returnVal.vkQueueSubmit = (PFN_vkQueueSubmit)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkQueueSubmit");
	returnVal.vkQueueWaitIdle = (PFN_vkQueueWaitIdle)PfnLoaderDeviceHelper(getDeviceProcAddr, device, "vkQueueWaitIdle");
	return returnVal;
}
	...
	PFN_vkGetInstanceProcAddr instanceProcAddr)
{
	...
	returnVal.vkCreateDebugUtilsMessengerEXT = (PFN_vkCreateDebugUtilsMessengerEXT)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkCreateDebugUtilsMessengerEXT");
	returnVal.vkDestroyDebugUtilsMessengerEXT = (PFN_vkDestroyDebugUtilsMessengerEXT)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkDestroyDebugUtilsMessengerEXT");
	returnVal.vkSetDebugUtilsObjectNameEXT = (PFN_vkSetDebugUtilsObjectNameEXT)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkSetDebugUtilsObjectNameEXT");
	return returnVal;
}
	...
	PFN_vkGetInstanceProcAddr instanceProcAddr)
{
	...
	returnVal.vkDestroySurfaceKHR = (PFN_vkDestroySurfaceKHR)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkDestroySurfaceKHR");
	returnVal.vkGetPhysicalDeviceSurfaceCapabilitiesKHR = (PFN_vkGetPhysicalDeviceSurfaceCapabilitiesKHR)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkGetPhysicalDeviceSurfaceCapabilitiesKHR");
	returnVal.vkGetPhysicalDeviceSurfaceFormatsKHR = (PFN_vkGetPhysicalDeviceSurfaceFormatsKHR)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkGetPhysicalDeviceSurfaceFormatsKHR");
	returnVal.vkGetPhysicalDeviceSurfacePresentModesKHR = (PFN_vkGetPhysicalDeviceSurfacePresentModesKHR)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkGetPhysicalDeviceSurfacePresentModesKHR");
	returnVal.vkGetPhysicalDeviceSurfaceSupportKHR = (PFN_vkGetPhysicalDeviceSurfaceSupportKHR)PfnLoaderInstanceHelper(instanceProcAddr, instance, "vkGetPhysicalDeviceSurfaceSupportKHR");
	return returnVal;
}
	...
	vk::SurfaceKHR surface) const
{
	...
	auto vkResult = static_cast<vk::Result>(surfaceRaw.vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
		static_cast<VkPhysicalDevice>(physDevice),
	...
	vk::SurfaceKHR surface) const
{
	...
	auto vkResult = static_cast<vk::Result>(surfaceRaw.vkGetPhysicalDeviceSurfaceSupportKHR(
		static_cast<VkPhysicalDevice>(physDevice),
		queueFamilyIndex,
		static_cast<VkSurfaceKHR>(surface),
		&surfaceSupported));
	...
	vk::Optional<vk::AllocationCallbacks> allocator) const
{
	...
	auto result = static_cast<vk::Result>(raw.vkAllocateMemory(
		static_cast<VkDevice>(handle),
...
{
	auto result = static_cast<vk::Result>(raw.vkBeginCommandBuffer(
...
{
	DENGINE_IMPL_GFX_ASSERT(commandBuffer != vk::CommandBuffer{});
	DENGINE_IMPL_GFX_ASSERT(layout != vk::PipelineLayout{});
...
{
	DENGINE_IMPL_GFX_ASSERT(Util::CheckNotNull(commandBuffer));
	DENGINE_IMPL_GFX_ASSERT(Util::CheckNotNull(pipeline));
...
void DeviceDispatch::cmdSetStencilReference(
	vk::CommandBuffer cmdBuffer,
	vk::StencilFaceFlags stencilFaceFlags,
	u32 value) const noexcept
{
	DENGINE_IMPL_GFX_ASSERT(cmdBuffer != vk::CommandBuffer{});
	raw.vkCmdSetStencilReference(
		static_cast<VkCommandBuffer>(cmdBuffer),
		static_cast<VkStencilFaceFlags>(stencilFaceFlags),
		value);
}
...
DeviceDispatch::CreateBoxVmaBufferResult DeviceDispatch::CreateBox(
	VmaAllocator const& vma,
	vk::BufferCreateInfo const& createInfo,
	VmaAllocationCreateInfo const& allocInfo) const
{
	DENGINE_IMPL_GFX_ASSERT(createInfo.size != 0);
	DENGINE_IMPL_GFX_ASSERT(createInfo.usage != vk::BufferUsageFlags{});
	DENGINE_IMPL_GFX_ASSERT(allocInfo.usage != 0);
	vk::Buffer bufferOut = {};
	...
	auto result = (vk::Result)vmaCreateBuffer(
		...
		&allocInfo,
		reinterpret_cast<VkBuffer*>(&bufferOut),
		&vmaAlloc,
		&vmaAllocInfo);
	if (result != vk::Result::eSuccess)
		throw std::runtime_error("Unable to allocate vk::Image");
	return CreateBoxVmaBufferResult {
		.buffer = BoxVmaBuffer::Adopt(vma, (VkBuffer)bufferOut, vmaAlloc),
		.allocInfo = vmaAllocInfo, };
}

vk::CommandPool DeviceDispatch::CreateCommandPool(
	...
	vk::Optional<vk::AllocationCallbacks> allocator) const
{
	...
	if (result != vk::Result::eSuccess)
		throw std::runtime_error("Unable to allocate Vulkan command pool");
	return temp;
}
...
{
	vk::Image temp;
	auto result = static_cast<vk::Result>(raw.vkCreateImage(
		static_cast<VkDevice>(handle),
...
DeviceDispatch::CreateBoxVmaImageResult DeviceDispatch::CreateBox(
	VmaAllocator const& vma,
	vk::ImageCreateInfo const& createInfo,
	VmaAllocationCreateInfo const& allocInfo) const
{
	...
	auto result = (vk::Result)vmaCreateImage(
		...
		reinterpret_cast<VkImageCreateInfo const*>(&createInfo),
		&allocInfo,
		reinterpret_cast<VkImage*>(&imageOut),
		&vmaAlloc,
		&vmaAllocInfo);
	if (result != vk::Result::eSuccess)
		throw std::runtime_error("Unable to allocate vk::Image");
	return CreateBoxVmaImageResult {
		.img = BoxVmaImg::Adopt(vma, imageOut, vmaAlloc),
		.allocInfo = vmaAllocInfo, };
}
	...
	vk::Optional<vk::AllocationCallbacks> allocator) const
{
	...
	auto result = static_cast<vk::Result>(raw.vkCreateShaderModule(
		static_cast<VkDevice>(handle),
...
vk::Pipeline DeviceDispatch::CreateGraphicsPipeline(vk::GraphicsPipelineCreateInfo const& info) const
{
	vk::Pipeline out = {};
	auto result = (vk::Result)raw.vkCreateGraphicsPipelines(
		static_cast<VkDevice>(handle),
		VkPipelineCache{},
		1,
		reinterpret_cast<VkGraphicsPipelineCreateInfo const*>(&info),
		nullptr,
		reinterpret_cast<VkPipeline*>(&out));
	if (result != vk::Result::eSuccess)
		throw std::runtime_error("DEngine - Vulkan: Unable to create Vulkan graphics pipeline");
	return out;
}
