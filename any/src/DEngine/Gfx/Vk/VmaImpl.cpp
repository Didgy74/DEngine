...
namespace DEngine::Gfx::Vk::Init
{
	static VKAPI_ATTR void VKAPI_CALL AllocCallbackForVMA(
		...
		void* pUserData)
	{
		// TODO: We can add a transient allocator to our pUserData and skip the allocation for
		// std::tring.
		VMA_MemoryTrackingData& trackingData = *static_cast<VMA_MemoryTrackingData*>(pUserData);

		if (trackingData.debugUtils != nullptr) {
			std::lock_guard _(trackingData.vma_idTracker_lock);

			std::string name = std::format("VMA DeviceMemory {}", trackingData.vma_idTracker);
...
VmaAllocator DEngine::Gfx::Vk::Init::InitializeVMA(
	...
	VMA_MemoryTrackingData* vma_trackingData)
{
	...
	if (vkResult != vk::Result::eSuccess)
		throw std::runtime_error("Unable to create VMA object");
	return returnVal;
}