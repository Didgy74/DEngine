...
{
	title = "PlayerMove";
	this->getThemeFn = Editor::WidgetThemeAdapters::collapsingHeader;
...
TransformWidget::TransformWidget(EditorImpl const& editorImpl)
{
	...
	outerGrid->getThemeFn = Editor::WidgetThemeAdapters::grid;
	...
	// Create the Position:
	{
		...
		positionLabel->getThemeFn = Editor::WidgetThemeAdapters::text;
		...
		for (int i = 0; i < 3; i += 1)
		{
			...
			inputField->getThemeFn = Editor::WidgetThemeAdapters::lineFloatEdit;
...

	{
		...
		rotationLabel->getThemeFn = Editor::WidgetThemeAdapters::text;
		...
		rotationInput->getThemeFn = Editor::WidgetThemeAdapters::lineFloatEdit;
		...
		rotationInput->valueChangedFn = [&editorImpl](LineFloatEdit& widget, f64 newValue) {
			...
			DENGINE_IMPL_ASSERT(componentPtr);
			auto& component = *componentPtr;
			component.rotation = Math::UnitQuat::FromEulerAngles(0.f, 0.f, (f32)newValue);
		};
	}

	{
		...
		scaleText->getThemeFn = Editor::WidgetThemeAdapters::text;
...
void TransformWidget::Update(ComponentType const& component)
{
	...
	if (!rotationInput->HasInputSession())
	{
		...
		widget.SetValue(component.rotation.ToEulerAngles().z);
	}
...
SpriteRenderer2DWidget::SpriteRenderer2DWidget(EditorImpl const& editorImpl)
{
	...
	textureIdLayout->getThemeFn = Editor::WidgetThemeAdapters::stackLayout;
	...
	textureIDLabel->getThemeFn = Editor::WidgetThemeAdapters::text;
	...
	textureIdInput->getThemeFn = Editor::WidgetThemeAdapters::lineIntEdit;
...
RigidbodyWidget::RigidbodyWidget(EditorImpl const& editorImpl) {
...

	{
		...
		bodyTypeLayout->getThemeFn = Editor::WidgetThemeAdapters::stackLayout;
		...
		bodyTypeLabel->getThemeFn = Editor::WidgetThemeAdapters::text;

		bodyTypeDropdown = new Dropdown;
		bodyTypeLayout->AddWidget(Std::Box{ bodyTypeDropdown });
		bodyTypeDropdown->getThemeFn = Editor::WidgetThemeAdapters::dropdown;
