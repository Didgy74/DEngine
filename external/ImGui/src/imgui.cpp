...
/*
 ...
 - 2019/XX/XX (1.XX) - when multi-viewportManager are enabled, all positions will be in your natural OS coordinates space. It means that:
 ...
 - 2018/06/08 (1.62) - examples: the imgui_impl_xxx files have been split to separate platform (Win32, Glfw, SDL2, etc.) from renderer (DX11, OpenGL, Vulkan,  etc.).
                       old bindings will still work as is, however prefer using the separated bindings as they will be updated to support multi-viewportManager.
...
bool ImGui::IsItemHovered(ImGuiHoveredFlags flags)
{
    ...
    // DestroyVulkanHppHandle for bounding box overlap, as updated as ItemAdd()
    if (!(window->DC.LastItemStatusFlags & ImGuiItemStatusFlags_HoveredRect))
        return false;
    IM_ASSERT((flags & (ImGuiHoveredFlags_RootWindow | ImGuiHoveredFlags_ChildWindows)) == 0);   // Flags not supported by this function

    // DestroyVulkanHppHandle if we are hovering the right window (our window could be behind another window)
    ...
    // DestroyVulkanHppHandle if another item is active (e.g. being dragged)
    if (!(flags & ImGuiHoveredFlags_AllowWhenBlockedByActiveItem))
        if (g.ActiveId != 0 && g.ActiveId != window->DC.LastItemId && !g.ActiveIdAllowOverlap && g.ActiveId != window->MoveId)
            return false;

    // DestroyVulkanHppHandle if interactions on this window are blocked by an active popup or modal.
    // The ImGuiHoveredFlags_AllowWhenBlockedByPopup flag will be tested here.
    if (!IsWindowContentHoverable(window, flags))
        return false;

    // DestroyVulkanHppHandle if the item is disabled
...
{
    // CreateJob the draw list on demand, because they are not frequently used for all viewportManager
    ImGuiContext& g = *GImGui;
...
void ImGui::NewFrame()
{
    ...
    // CreateJob implicit/fallback window - which we will only render it if the user has added something to it.
...
static void ImGui::EndFrameDrawDimmedBackgrounds()
{
    ...
    // Draw modal whitening background on _other_ viewportManager than the one the modal is one
    ImGuiWindow* modal_window = GetTopMostPopupModal();
...
// DestroyVulkanHppHandle if mouse cursor is hovering given rectangle
...
static ImGuiWindow* CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
    ...
    // CreateJob window the first time
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Draw modal or window list full viewport dimming background (for other viewportManager we'll render them in EndFrame)
...
{
    // CreateJob switch-case from enum with regexp: ImGuiCol_{.*}, --> case ImGuiCol_\1: return "\1";
    switch (idx)
    {
...
{
    // DestroyVulkanHppHandle condition (NB: bit 0 is always true) and clear flags for next time
...
// (r_outer is usually equivalent to the viewport rectangle minus padding, but when multi-viewportManager are enabled and monitor
...
{
    // Tooltips and menus are not automatically forced into their own viewport when the NoMerge flag is set, however the multiplication of viewportManager makes them more likely to protrude and create their own.
    ImGuiContext& g = *GImGui;
...
// Update viewportManager and monitor infos
// Note that this is running even if 'ImGuiConfigFlags_ViewportsEnable' is not set, in order to clear unused viewportManager (if any) and update monitor info.
static void ImGui::UpdateViewportsNewFrame()
{
    ...
    // CreateJob/update main viewport with current platform position. 
    ...
    // - when we are between viewportManager, our dragged preview will tend to show in the last viewport _even_ if we don't have tooltips in their viewportManager (when lacking monitor info)
...
ImGuiViewportP* ImGui::AddUpdateViewport(ImGuiWindow* window, ImGuiID id, const ImVec2& pos, const ImVec2& size, ImGuiViewportFlags flags)
{
    ...
    else
    {
        ...
        // We normally setup for all viewportManager in NewFrame() but here need to handle the mid-frame creation of a new viewport.
...
void ImGui::UpdatePlatformWindows()
{
    ...
    // CreateJob/resize/destroy platform windows to match each active viewport.
    ...
    for (int i = 1; i < g.Viewports.Size; i++)
    {
        ...
        {
            IMGUI_DEBUG_LOG_VIEWPORT("CreateJob Platform Window %08X (%s)\n", viewport->ID, viewport->Window ? viewport->Window->Name : "n/a");
...
void ImGui::DockContextUpdateDocking(ImGuiContext* ctx)
{
    ...
    // CreateJob windows for each automatic docking nodes
...
void ImGui::DockContextProcessDock(ImGuiContext* ctx, ImGuiDockRequest* req)
{
    ...
    // CreateJob new node and add existing window to it
    if (node == NULL)
    {
    ...
    {
        // CreateJob tab bar before we call DockNodeMoveWindows (which would attempt to move the old tab-bar, which would lead us to payload tabs wrongly appearing before target tabs!)
...
void ImGui::DockContextProcessUndockNode(ImGuiContext* ctx, ImGuiDockNode* node)
{
    ...
    {
        // In the case of a root node or central node, the node will have to stay in place. CreateJob a new node to receive the payload.
...
static void ImGui::DockNodeAddWindow(ImGuiDockNode* node, ImGuiWindow* window, bool add_to_tab_bar)
{
    ...
    // In particular it is important we init the viewport from the first window so we don't create two viewportManager and drop one.
...
static void ImGui::DockNodePreviewDockRender(ImGuiWindow* host_window, ImGuiDockNode* host_node, ImGuiWindow* root_payload, const ImGuiDockPreviewData* Data)
{
    ...
    // In case the two windows involved are on different viewportManager, we will draw the overlay on each of them.
...
// CreateJob an explicit dockspace node within an existing window. Also expose dock node flags and creates a CentralNode by default.
...
void ImGui::ShowViewportThumbnails()
{
    ...
    // We don't display full monitor bounds (we could, but it often looks awkward), instead we display just enough to cover all of our viewportManager.
