...
module DEngine.Std.BumpAllocator;
...
public:
	...
	static void PushToBlockList(BlockList& list, Block block) {
		DENGINE_IMPL_CONTAINERS_ASSERT(list.count <= list.capacity);
		DENGINE_IMPL_CONTAINERS_ASSERT(block.data != nullptr);
...
namespace DEngine::Std
{
	static auto CalcAlignedOffset(void const* ptr, uSize offset, uSize alignment) {
		...
		auto const alignedAsInt = Std::CeilToMultiple(asInt, (u64)alignment);
...
void* Std::BumpAllocator::Alloc(uSize allocSize, uSize allocAlignment) noexcept
{
...

	{
		...
		auto const alignedOffset = CalcAlignedOffset(
			activeBlock.data,
			activeBlock.offset,
			Std::Max(allocAlignment, Impl::minAllocAlignment));
		...
		if (alignedOffset + allocSize > activeBlock.size) {
			...
			auto newSize = Std::Max(allocSize, activeBlock.size * 2);
	...
	DENGINE_IMPL_CONTAINERS_ASSERT(activeBlock.data != nullptr);
	auto const alignedOffset = CalcAlignedOffset(
		activeBlock.data,
		activeBlock.offset,
		Std::Max(allocAlignment, Impl::minAllocAlignment));
	DENGINE_IMPL_CONTAINERS_ASSERT(alignedOffset + allocSize <= activeBlock.size);
...
bool Std::BumpAllocator::IsEmpty() const noexcept {
	if (activeBlock.offset == 0)
		return true;
	return false;
}