...
export module DEngine.Std.StackVec;
...
export namespace DEngine::Std {
	...
	public:
			...
			: size(in.size)
		{
			for (int i = 0; i < size; i++)
				new(values + i, Std::placementNewTag) T(in[i]);
		}
		...
		{
			for (int i = 0; i < size; i++)
				new(values + i, Std::placementNewTag) T(static_cast<T&&>(in[i]));
			in.Clear();
		}
		...
		StackVec(Span<T const> const& in) noexcept
			requires (Trait::isCopyConstructible<T>)
			: size{ in.Size() }
		{
			DENGINE_IMPL_CONTAINERS_ASSERT(in.Size() <= capacity);
			int i = 0;
			for (auto const& inItem : in) {
				new(values + i, Std::placementNewTag) T(inItem);
				i++;
			}
		}
		template<uSize otherSize>
		StackVec(T const (&arr)[otherSize]) noexcept requires (Trait::isCopyConstructible<T> && otherSize <= capacity)
			: size{ otherSize } {
			for (int i = 0; i < otherSize; i++)
				new(values + i, Std::placementNewTag) T(arr[i]);
		}
		~StackVec();
		...
		//constexpr operator Span<T>() noexcept { return ToSpan(); }
		//constexpr operator Span<T const>() const noexcept { return ToSpan(); }

		[[nodiscard]] constexpr bool CanPushBack() const noexcept;
	...
	private:
		union {
			T values[capacity];
			alignas(T) unsigned char _unused;
		};
		uSize size = 0;
	};
	...
	template<class T, unsigned int capacity>
	constexpr bool StackVec<T, capacity>::CanPushBack() const noexcept {
