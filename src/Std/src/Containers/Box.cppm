module;

#include <DEngine/Std/Containers/impl/Assert.hpp>

export module DEngine.Std.Box;

import DEngine.Std.Trait;

export namespace DEngine::Std {
	template<class T>
	class Box;

	template<class T>
	[[nodiscard]] Box<T> BoxAdopt(T* ptr) noexcept;

	/**
		TODO: In the future, maybe make it illegal to have an empty Box object?
	 */
	template<class T>
	class Box {
	public:
		template<class U>
		friend class Box;

		using ValueType = T;

		constexpr Box() noexcept : data{ nullptr } {}

		constexpr Box(decltype(nullptr)) noexcept : data(nullptr) {}
		// To disable accidental implicit conversion from integer to nullptr.
		explicit Box(int) = delete;

		Box(Box const&) = delete;

		constexpr Box(Box&& other) noexcept :
			data { other.data } {
			other.data = nullptr;
		}

		template<class U> requires (Trait::hasVirtualDestructor<T>)
		constexpr Box(Box<U>&& in) noexcept :
			data{ in.data } {
			in.data = nullptr;
		}

		// Should probably be deprecated to in favor of BoxAdopt?
		explicit constexpr Box(T* ptr) noexcept : data{ ptr } {}

		~Box() {
			Clear();
		}

		Box& operator=(Box const&) = delete;
		Box& operator=(Box&&) noexcept;
		template<class U> requires (Trait::hasVirtualDestructor<T>)
		constexpr Box& operator=(Box<U>&& in) noexcept
		{
			Clear();
			data = in.data;
			in.data = nullptr;
			return *this;
		}
		Box& operator=(decltype(nullptr)) noexcept;

		[[nodiscard]] T* Get() noexcept;
		[[nodiscard]] T const* Get() const noexcept;
		[[nodiscard]] T* operator->() noexcept;
		[[nodiscard]] T const* operator->() const noexcept;
		[[nodiscard]] T& operator*() noexcept;
		[[nodiscard]] T const& operator*() const noexcept;

		[[nodiscard]] explicit operator bool() const noexcept;
		[[nodiscard]] bool Has() const noexcept { return this->data != nullptr; }

		/*
		[[nodiscard]] bool operator==(Box const&) const noexcept;
		[[nodiscard]] bool operator==(T const*) const noexcept;
		*/

		// Returns the pointer and releases ownership over it.
		[[nodiscard]] T* Release() noexcept;
		// Destroys the internals if it holds any.
		void Clear() noexcept;

	protected:
		T* data = nullptr;

		template<class U>
		friend Box<U> BoxAdopt(U* ptr) noexcept;
	};

	template<class T>
	inline Box<T>& Box<T>::operator=(
		Box&& right) noexcept
	{
		Clear();
		data = right.data;
		right.data = nullptr;

		return *this;
	}

	template<class T>
	inline Box<T>& Box<T>::operator=(decltype(nullptr)) noexcept {
		Clear();
		data = nullptr;
		return *this;
	}

	template<class T>
	inline T* Box<T>::Get() noexcept {
		return data;
	}

	template<class T>
	inline T const* Box<T>::Get() const noexcept {
		return data;
	}

	template<class T>
	inline T* Box<T>::operator->() noexcept {
		return data;
	}

	template<class T>
	inline T const* Box<T>::operator->() const noexcept {
		return data;
	}

	template<class T>
	inline T& Box<T>::operator*() noexcept {
		DENGINE_IMPL_CONTAINERS_ASSERT_MSG(
			data != nullptr,
			"Attempted to dereference a Std::Box with a nullptr value.");
		return *data;
	}

	template<class T>
	inline T const& Box<T>::operator*() const noexcept {
		DENGINE_IMPL_CONTAINERS_ASSERT_MSG(
			data != nullptr,
			"Attempted to dereference a Std::Box with a nullptr value.");
		return *data;
	}

	template<class T>
	inline Box<T>::operator bool() const noexcept {
		return data != nullptr;
	}

	/*
	template<class T>
	inline bool Box<T>::operator==(T const* other) const noexcept
	{
		return data == other;
	}
	 */

	template<class T>
	inline T* Box<T>::Release() noexcept {
		T* returnVal = data;
		data = nullptr;
		return returnVal;
	}

	template<class T>
	inline void Box<T>::Clear() noexcept {
		if (data) {
			delete data;
			data = nullptr;
		}
	}

	template<class T>
	[[nodiscard]] Box<T> BoxAdopt(T* ptr) noexcept {
		Box<T> returnVal;
		returnVal.data = ptr;
		return returnVal;
	}
}
