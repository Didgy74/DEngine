...
export namespace DEngine::Std {
	...
	public:
		...
		void PushBack(Std::Span<T const> const& input) requires (Trait::isCopyConstructible<T>) {
			...
			if (count + inputSize > capacity) {
				Grow(NextCapacity(count + inputSize));
			}
			for (int i = 0; i < inputSize; i++) {
				new(data + count + i, Std::placementNewTag) T(input[i]);
			}

			count += inputSize;
		}

		void PushBack(T const& input) noexcept requires (Trait::isCopyConstructible<T>) {
			DENGINE_IMPL_CONTAINERS_ASSERT(count <= capacity);
			if (count == capacity) {
				Grow(NextCapacity(count + 1));
			}
			new(data + count, Std::placementNewTag) T(input);
			count += 1;
		}
		...
		void PushBack(T&& in) noexcept {
			...
			new(data + count, Std::placementNewTag) T(static_cast<T&&>(in));
			count += 1;
		}
			...
			if (newSize > count) {
				if constexpr (!Trait::isTriviallyDefaultConstructible<T>) {
					for (uSize i = count; i < newSize; i += 1) {
						new(data + i, Std::placementNewTag) T;
					}
				}
			}
		...
		void Resize(uSize newSize, T const& newValue) noexcept requires (Trait::isCopyConstructible<T>) {
			...
			if (newSize > count) {
				for (uSize i = count; i < newSize; i += 1) {
					new(data + i, Std::placementNewTag) T(newValue);
				}
			}
			else if (newSize < count) {
				if constexpr (!Trait::isTriviallyDestructible<T>) {
		...
		/**
			Erases a value at index i while preserving order. Slower than EraseUnsorted().
		 */
		...
		/**
			Erases a value at index i without preserving order. Faster than Erase().
		 */
	...
	protected:
		...
		[[nodiscard]] uSize NextCapacity(uSize minRequired) const noexcept {
			auto doubled = capacity > 0 ? capacity * 2 : 4;
			return doubled > minRequired ? doubled : minRequired;
		}
			...
			if (needNewAlloc) {
				...
				if (oldData) {
					for (uSize i = 0; i < count; i++) {
						new(data + i, Std::placementNewTag) T(static_cast<T&&>(oldData[i]));
						if constexpr (!Trait::isTriviallyDestructible<T>) {
							oldData[i].~T();
						}
					}
					alloc.Free(oldData, oldCapacity * sizeof(T));
				}
			}
	...
	template<class T, class AllocRefT>
	inline auto NewVec_Reserve(AllocRefT const& alloc, uSize size) {
		auto temp = Std::Vec<T, AllocRefT>{ alloc };
		temp.Reserve(size);
		return temp;
	}
