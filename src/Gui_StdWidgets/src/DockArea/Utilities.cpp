	...
	Extent const& gizmoSize) noexcept
{
	...
	auto offsetX = (int)Std::Floor((f32)gizmoSize.width * gizmoOffsetFactor);
	auto offsetY = (int)Std::Floor((f32)gizmoSize.height * gizmoOffsetFactor);
	...
	Extent const& gizmoSize)
{
	...
	returnVal.position.y -= (int)Std::Floor((f32)gizmoSize.height * gizmoOffsetFactor) * 2;
	return returnVal;
}
	...
	DA_SplitNode::Direction dir) noexcept
{
	...
	if (isHoriz) {
		returnVal.position.x += (i32)Std::Round((f32)nodeRect.extent.width * splitOffset);
		...
		returnVal.position.x += (i32)nodeRect.extent.width / 2;
		returnVal.position.x -= (i32)returnVal.extent.width / 2;
		returnVal.position.y += (i32)Std::Round((f32)nodeRect.extent.height * splitOffset);
		returnVal.position.y -= (i32)returnVal.extent.height / 2;
	}
...
auto Gui::impl::DA_BuildLayerItPairFrontToBack(DockArea& dockArea) noexcept -> DA_LayerItPair
{
...
auto Gui::impl::DA_BuildLayerItPairFrontToBack(DockArea const& dockArea) noexcept -> DA_LayerConstItPair
{
	...
	u32 totalTabHeight) -> DA_WindowNodePrimaryRects
{
	...
	returnValue.contentRect.extent.height = (u32)Std::Max(
		0,
	...
	AllocRef transientAlloc)
{
	...
	for (int i = 0 ; i < tabCount ; i += 1) {
		...
		auto outerExtent = textManager.GetOuterExtent(
			{ text.data(), text.size() },
			fontSizeId,
			Gui::TextHeightType::Alphas,
			{});
		desiredWidth = (int)outerExtent.width + 2 * horiMarginAmountPx;
	}
	...
	for (int i = 0 ; i < tabCount ; i += 1) {
		...
		if (sizeRatio >= 1.0f) {
			...
			if (i == (tabCount - 1))
				...
				width = (int)Std::Round((f32)desiredWidths[i] * sizeRatio);
		}
...
namespace DEngine::Gui::impl {
		...
		Widget::Render_Params const& renderParams)
	{
		...
		drawInfo.PushFilledQuad(visibleIntersection, contentBackgroundColor);

		if (!tab.widget.Has())
			return;

		auto scissor = DrawEngine::ScopedScissor(drawInfo, visibleIntersection);
...
{
	auto& textManager = params.widgetRenderParams.textEngine;
	auto& transientAlloc = params.widgetRenderParams.transientAlloc;
	auto& drawInfo = params.widgetRenderParams.drawEngine;
	...
	auto tabTotalHeightPx = params.tabTotalHeightPx;
	auto fontSizeId = params.fontSizeId;
	auto tabMarginAmountPx = params.textMarginAmountPx;
	auto tabCornerRadiusPx = params.tabCornerRadiusPx;
	auto isBackLayer = params.isBackLayer;

	auto [titlebarRect, contentRect] =
		impl::DA_WindowNode_BuildPrimaryRects(nodeRect, tabTotalHeightPx);

	DENGINE_IMPL_GUI_ASSERT(!tabs.empty());
	auto& activeTab = tabs[activeTabIndex];

	// TODO: Optimize RectColl lookup?

	auto contentIntersection = Intersection(contentRect, visibleRect);
	if (!contentIntersection.IsNothing()) {
		Render_WindowNode_Child(
			activeTab,
			contentIntersection,
			Std::nullOpt,
	...
	Render_WindowNode_TitlebarBackground(activeTab.color, titlebarRect, 0, drawInfo);
	...
	for (int i = 0; i < (int)tabs.size(); i += 1) {
		...
		drawInfo.PushFilledQuad(tabRect, color, { (i32)tabCornerRadiusPx, 0, 0, (i32)tabCornerRadiusPx });
		...
		auto scissor = DrawEngine::ScopedScissor(drawInfo, textRect, tabRect);
