...
namespace DEngine::Platform::impl::Backend {
	...
	struct CustomJobItem {
		u64 id = 0;
		void* data = nullptr;
		using ConsumeFnT = void (*)(void*, Context::Impl&, BackendData&);
	...
	struct BackendData : public Context::Impl::PlatformBackendBase {
		...
		// XInput connection state. Bit N is set when user-index N is connected.
		// Connected slots are polled every frame; disconnected slots are probed
		// only every xinputDisconnectedPollInterval frames to avoid wasting cycles.
		u8 xinputConnectedMask = 0;
		u32 xinputDisconnectedPollCounter = 0;
		static constexpr u32 xinputDisconnectedPollInterval = 60;
	};

	// TODO: Review how this should be used
	...
	// Thread safe
	void RunOnBackendThread_Inner(
		BackendData& backendData,
		RunOnBackendThread_JobItem item);
	// Returns a future with the return value of the lambda
	// Thread safe
	template<class Callable> requires 
		std::invocable<Callable, Context::Impl&, BackendData&> && 
		Std::Trait::isMoveConstructible<Callable>
	auto RunOnBackendThread(BackendData& backendData, Callable&& func)
	{
		using RType = std::invoke_result_t<Callable, Context::Impl&, BackendData&>;
		struct JobData {
			Callable func;
		...
		jobItem.consumeFn = [](void* dataIn, Context::Impl& implData, BackendData& backendData) {
			auto* jobData = (JobData*)dataIn;
			if constexpr (Std::Trait::isSame<RType, void>) {
				jobData->func(implData, backendData);
				jobData->promise.set_value();
			}
			else {
				jobData->promise.set_value(jobData->func(implData, backendData));
			}
			delete jobData;
			};
		RunOnBackendThread_Inner(backendData, jobItem);

		return future;
	}

	// Pushes a job to the polling-thread and waits for it to finish, then returns the
	// result.
	// 
	// Thread safe
	template<class Callable> requires std::invocable<Callable, Context::Impl&, BackendData&>
	auto RunOnBackendThread_Wait(BackendData& backendData, Callable&& func)
	{
		auto tempFuture = RunOnBackendThread(backendData, Std::Move(func));
		...
		public IRawElementProviderFragment
	{
		...
		HRESULT QueryInterface(REFIID riid, void** ppvObject) override {
			DENGINE_IMPL_APPLICATION_ASSERT(__uuidof(IUnknown) == IID_IUnknown);
		...
		public ITfTextEditSink
	{
			...
			if (character == '\b') {
				...
				*/
				...
				PushEventJob_ThreadSafe(
					backendData,
					[=](Context& ctx) {
						auto characterCasted = (u32)character;
						BackendInterface::PushTextInputEvent(
							ctx.GetImplData(),
							windowId,
							oldSelIndex,
							oldSelCount,
							Std::Span{ characterCasted });
					});
		...
		HRESULT SetSelection(ULONG ulCount, TS_SELECTION_ACP const* pSelection) override
		{
			...
			RunOnBackendThread(
				*this->backendData,
				[=](Context::Impl& implData, BackendData& backend) {
					BackendInterface::PushTextSelectionEvent(
						implData,
						windowId,
						(u64)newSelIndex,
			...
			TS_TEXTCHANGE* out_change) override
		{
			...
			*/
			...
			RunOnBackendThread(
				*this->backendData,
				[=, outputText = Std::Move(outputText)](Context::Impl& implData, BackendData& backend) {
					BackendInterface::PushTextInputEvent(
						implData,
						windowId,
						(u64)acpStart,
			...
			ULONG* out_AttrValsFetched) override
		{
			...
			*out_AttrValsFetched = Std::Min((ULONG)this->requestedAttrs.size(), attrValsCapacity);
