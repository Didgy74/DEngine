...
export module DEngine.Gui.Utility;
...
export namespace DEngine::Gui {
	...
	// Given an angle [0, 1] where 0 means up and 1 means a full revolve. Positive values rotate
	// counter-clockwise around the Z axis.
	// Quadrant checks: each direction covers ±45° (0.25 of the circle).
	[[nodiscard]] constexpr bool DirAngleIsQuadrantUp(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle <= 0.125f || angle >= 0.875f;
	}
	[[nodiscard]] constexpr bool DirAngleIsQuadrantLeft(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle >= 0.125f && angle <= 0.375;
	}
	[[nodiscard]] constexpr bool DirAngleIsQuadrantDown(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle >= 0.375f && angle <= 0.625f;
	}
	[[nodiscard]] constexpr bool DirAngleIsQuadrantRight(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle >= 0.625f && angle <= 0.875f;
	}

	// Octant checks: each direction covers 22.5° (0.125 of the circle).
	// Use these when there is no immediate neighbor in that direction, requiring
	// a more deliberate joystick push to cross the layout boundary.
	[[nodiscard]] constexpr bool DirAngleIsOctantUp(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle <= 0.0625f || angle >= 0.9375f;
	}
	[[nodiscard]] constexpr bool DirAngleIsOctantLeft(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle >= 0.1875f && angle <= 0.3125f;
	}
	[[nodiscard]] constexpr bool DirAngleIsOctantDown(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle >= 0.4375f && angle <= 0.5625f;
	}
	[[nodiscard]] constexpr bool DirAngleIsOctantRight(f32 angle) {
		DENGINE_IMPL_ASSERT(angle >= 0 && angle <= 1);
		return angle >= 0.6875f && angle <= 0.8125f;
	}
	[[nodiscard]] inline Math::Vec2 NavDirAngleToVec2(f32 angle) {
		f32 const angleRadians = angle * Math::pi * 2.f;
		return {
			-Math::Sin(angleRadians),
			-Math::Cos(angleRadians) };
	}

	struct FindBestNavItem2_ReturnT {
		int index = 0;
	};
	/**
		This function finds the best navigation item for a given focus widget and navigation angle.
		It iterates through a range of navigation items, calculates scores based on angle and
	   distance, and returns the best item found or null if no valid child is found.
	 */
	[[nodiscard]] Std::Opt<FindBestNavItem2_ReturnT> FindBestNavItem2(
	...
	struct Rect {
		...
		[[nodiscard]] constexpr Rect WithOffset(Math::Vec2Int offset) const noexcept;

		[[nodiscard]] inline Math::Vec2Int CenterPoint() const noexcept;
		...
		template<int N>
		[[nodiscard]] inline static Rect Intersection(Rect const (&in)[N]) noexcept;

		[[nodiscard]] constexpr bool operator==(Rect const&) const noexcept;
		[[nodiscard]] constexpr bool operator!=(Rect const&) const noexcept;
	};

	// We intentionally do not implement addition because we cannot represent multiple units
	// and "scaleWithContent" combinations without a complex data structure.
	struct Distance {
		enum class Unit { Px, Mm };
		...
		[[nodiscard]] constexpr Unit GetUnit() const noexcept { return m_unit; }
		[[nodiscard]] f64 GetFloat() const noexcept {
			DENGINE_IMPL_ASSERT(m_unit == Unit::Mm);
			return m_float;
		}

		[[nodiscard]] static constexpr Distance Mm(f32 input, bool scaleWithContent) noexcept {
			DENGINE_IMPL_ASSERT(input >= 0);
			auto temp = Distance();
			temp.m_unit = Unit::Mm;
			temp.m_float = input;
			temp.m_scaleWithContent = scaleWithContent;
			return temp;
		}

		[[nodiscard]] static constexpr Distance Px(u32 input, bool scaleWithContent) noexcept {
			auto temp = Distance();
			temp.m_unit = Unit::Px;
			temp.m_int = input;
			temp.m_scaleWithContent = scaleWithContent;
		...
		[[nodiscard]] u32 ResolvePx(f32 dpi, f32 contentScale) const noexcept {
			auto scale = m_scaleWithContent ? (f64)contentScale : 1.0;
			switch (m_unit) {
				case Unit::Px: return (u32)Std::Round((f64)m_int * scale);
				case Unit::Mm: return (u32)Std::Round(m_float * (dpi / 25.4) * scale);
				default: DENGINE_IMPL_UNREACHABLE();
			}
		}

		[[nodiscard]] constexpr Distance Multiply(f32 input) const {
			auto temp = *this;
			switch (m_unit) {
				case Unit::Px:
					temp.m_int = (u32)Std::Round((f64)temp.m_int * input);
					break;
				case Unit::Mm:
					temp.m_float *= input;
					break;
	...
	private:
		Unit m_unit;
		union {
			// TODO: We should likely just use a single double for all cases, to better
			// represent in between computations.
			u32 m_int;
			f64 m_float;
		};
		bool m_scaleWithContent = false;
	};

	[[nodiscard]] constexpr Distance operator*(Distance const& lhs, f32 rhs) noexcept { return lhs.Multiply(rhs); }
	[[nodiscard]] constexpr Distance operator*(f32 rhs, Distance const& lhs) noexcept { return lhs.Multiply(rhs); }
	...
	[[nodiscard]] inline auto CenterRangeOffset(int outer, int inner) noexcept {
		return (i32)Math::Round((f64)outer * 0.5 - (f64)inner * 0.5);
	}
...
constexpr DEngine::Gui::Rect DEngine::Gui::Rect::WithOffset(Math::Vec2Int offset) const noexcept {
	return { this->position + offset, this->extent };
}

DEngine::Math::Vec2Int DEngine::Gui::Rect::CenterPoint() const noexcept {
	return {
		position.x + (i32)Math::Round((f32)extent.width / 2.f),
		position.y + (i32)Math::Round((f32)extent.height / 2.f), };
}
...
template<int N>
inline DEngine::Gui::Rect DEngine::Gui::Rect::Intersection(Rect const (&in)[N]) noexcept
{
	return Intersection({ in, N });
}
