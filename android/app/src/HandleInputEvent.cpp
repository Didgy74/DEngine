...
namespace DEngine::Platform::impl::Backend
{
	[[nodiscard]] static GamepadKey ToGamepadButton(int32_t androidKeyCode) noexcept
	{
		...
		{
			case AKEYCODE_BUTTON_A:      return GamepadKey::A;
			case AKEYCODE_BUTTON_B:      return GamepadKey::B;
			case AKEYCODE_BUTTON_X:      return GamepadKey::X;
			case AKEYCODE_BUTTON_Y:      return GamepadKey::Y;
			case AKEYCODE_BUTTON_L1:     return GamepadKey::L1;
			case AKEYCODE_BUTTON_L2:     return GamepadKey::L2;
			case AKEYCODE_BUTTON_THUMBL: return GamepadKey::L3;
			case AKEYCODE_BUTTON_R1:     return GamepadKey::R1;
			case AKEYCODE_BUTTON_R2:     return GamepadKey::R2;
			case AKEYCODE_BUTTON_THUMBR: return GamepadKey::R3;
		}
		...
		AInputEvent const* event)
	{
		...
		// Only fire on actual press/release transitions. This drops key auto-repeat
		// (held buttons spam ACTION_DOWN) and redundant events, matching the dedupe
		// the polling backends do against gamepadState.keyStates.
		bool const pressed = action == AKEY_EVENT_ACTION_DOWN;
		if (implData.gamepadState.keyStates[(int)gamepadButton] != pressed) {
			auto const deviceId = (GamepadDeviceId)AInputEvent_getDeviceId(event);
			BackendInterface::UpdateGamepadKey(implData, deviceId, gamepadButton, pressed);
		}
		...
		i32 index)
	{
		...
		{
			// Input can arrive before the app has created its window; ignore until then.
		...
		i32 action)
	{
		...
		auto const deviceId = (GamepadDeviceId)AInputEvent_getDeviceId(event);
		// Android reports joystick Y as +down/near; the engine convention is +up, so we
		// negate both stick Y axes. The right stick maps to the Z/RZ axes by convention.
		BackendInterface::UpdateGamepadAxis(implData, deviceId, GamepadAxis::LeftX,   AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_X,  0));
		BackendInterface::UpdateGamepadAxis(implData, deviceId, GamepadAxis::LeftY,  -AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_Y,  0));
		BackendInterface::UpdateGamepadAxis(implData, deviceId, GamepadAxis::RightX,  AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_Z,  0));
		BackendInterface::UpdateGamepadAxis(implData, deviceId, GamepadAxis::RightY, -AMotionEvent_getAxisValue(event, AMOTION_EVENT_AXIS_RZ, 0));
...
// This function is called from the AInputQueue function when we poll the ALooper
// Should return true if the event has been handled by the app.
bool Backend::HandleInputEvent_Motion_Touch(
	Context::Impl& implData,
	BackendData& backendData,
	AInputEvent const* event)
{
	...
	// Touches can arrive before the app has created its window; ignore until then.
	if (!backendData.currWindowId.Has())
		return false;
...
// This function is called from the AInputQueue function when we poll the ALooper
// Should return true if the event has been handled by the app.
bool Backend::HandleInputEvent_Motion(
...
int Backend::looperCallback_InputEvent(int fd, int events, void* data)
{
